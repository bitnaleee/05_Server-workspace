...
public class PizzaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1) 요청시 전달값 뽑기 및 데이터가공처리(파싱같은거) => 변수 및 객체 기록
		//    (만일 post방식요청일 경우 전달값 뽑기 전 인코딩 처리)
		String name = request.getParameter("userName"); // "강해린"
		...
		// 3) 반환받은 결과를 가지고 사용자가 보게 될 응답페이지 만들기 혹은 jsp 위임
